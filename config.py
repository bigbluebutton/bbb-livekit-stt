...
class GladiaConfig:
    ...
    code_switching: bool | None = field(
        default_factory=lambda: _get_bool_env("GLADIA_CODE_SWITCHING", None)
    )
    ...
    custom_vocabulary: List[str] | None = field(
        default_factory=lambda: _get_list_env("GLADIA_CUSTOM_VOCABULARY", None)
    )
    ...
    pre_processing_speech_threshold: float | None = field(
        default_factory=lambda: _get_float_env(
            "GLADIA_PRE_PROCESSING_SPEECH_THRESHOLD", None
        )
    )
