...
async def entrypoint(ctx: JobContext):
    ...
    async def on_redis_message(message_data: str):
        try:
            ...
            evt_name = envelope.get("name")

            if evt_name == RedisManager.USER_SPEECH_LOCALE_CHANGED_EVT_MSG:
            ...
            elif evt_name == RedisManager.USER_SPEECH_OPTIONS_CHANGED_EVT_MSG:
                if meeting_id != agent.room.name:
                    return

                partial_utterances = body.get("partialUtterances", False)
                min_utterance_length = int(body.get("minUtteranceLength", 0))
                agent.update_speech_options_for_user(user_id, partial_utterances, min_utterance_length)
    ...
    async def on_interim_transcript(participant: rtc.RemoteParticipant, event: stt.SpeechEvent):
        ...
        for alternative in event.alternatives:
            ...
            if min_utterance_length and len(text.split()) < min_utterance_length:
                logging.debug(f"Interim transcript for {participant.identity} too short ({len(text.split())} words < {min_utterance_length}), skipping.")
                continue
            ...
            await redis_manager.publish_update_transcript_pub_msg(
                agent.room.name, participant.identity, alternative, bbb_locale, is_final=False
            )
