    ...
    async def on_redis_message(message_data: str):
        try:
            ...
            if evt_name == RedisManager.USER_SPEECH_LOCALE_CHANGED_EVT_MSG:
                ...
                agent.update_speech_options_for_user(
                    user_id, partial_utterances, min_utterance_length
                )
    ...
    async def on_interim_transcript(
        ...
        for alternative in event.alternatives:
            ...
            if min_utterance_length and len(text.split()) < min_utterance_length:
                logging.debug(
                    f"Interim transcript for {participant.identity} too short ({len(text.split())} words < {min_utterance_length}), skipping."
                )
                continue
